...
`src/modeling.py` establishes the first reproducible predictive baseline. It keeps the model-ready table in chronological order, assigns the earliest 80% to training and the latest 20% to a single untouched holdout, and uses only the final 20% of the training interval to select among standardized OLS and Ridge regressions. After selection by validation MAE, the winning candidate is refit on the complete training interval. Scaling is always fitted inside the model pipeline, so holdout distribution information cannot influence preprocessing.

The model uses 13 of the 14 Stage 09 predictors: current and lagged return, absolute return, normalized intraday range, 5- and 21-session rolling volatility, 5-session mean absolute return, volume change, relative volume, and Monday–Thursday indicators. Friday is deliberately omitted as the calendar reference category, preventing perfect multicollinearity between five weekday dummies and the intercept. These features are available by the current close and encode recent risk, price range, liquidity/attention, and calendar effects. Standardization makes coefficients comparable across units, while Ridge variations provide an automated response to correlated rolling-risk predictors. Standardized coefficients are conditional associations—not causal effects—and correlated predictors may share or reverse weight.

Held-out evaluation reports MAE as the primary metric, plus RMSE, R², and MAE on high-volatility observations defined using the training target's 80th percentile. The same dates are scored with `rolling_vol_21d` as a transparent benchmark, and the notebook reports MAE improvement or deterioration relative to it. Predictions are clipped at zero because the target is a magnitude. `reports/aapl_regression_diagnostics.png` contains predicted-versus-actual, residual-versus-fitted, residual-over-time, and normal Q–Q views; validation, holdout, and coefficient tables are also saved under `reports/`.

The linear specification assumes reasonably stable, approximately linear conditional relationships and well-behaved errors. Volatility data can violate normality, constant variance, independence, and coefficient stability through heavy tails and regime shifts. Diagnostics reveal these risks but do not remove them. This first split is an educational baseline, not evidence of deployable stability; repeated expanding-window validation, tail underprediction review, and monitoring remain required before operational use. Forecasts support analyst review and must not trigger automated trades or hedges.

In the executed Stage 10a notebook, chronological validation selected Ridge with `alpha=100`. On the 330-session holdout from 2025-04-30 through 2026-08-21, its MAE was 0.008453 versus 0.010488 for the rolling-volatility baseline, a 19.40% average improvement. However, out-of-sample R² was -0.015 and model tail MAE was 0.020958 versus the baseline's 0.019124, about 9.6% worse on the training-defined high-volatility subset. The largest positive standardized coefficient was current intraday range; the largest negative coefficient was the prior session's signed return. These results suggest useful average-error compression but weak explanatory power and inferior shock-period performance, so the provisional 5% average-MAE goal is not sufficient for operational acceptance.

## Stage 10b Expanding-Window Evaluation

Stage 10b retains the regression track and tests whether Stage 10a's result persists through time. `expanding_window_backtest()` begins with 600 chronological training observations and evaluates successive non-overlapping blocks of up to 200 sessions. Each test observation is forecast exactly once. Within every fold, OLS and Ridge variations are selected on the historical training interval, then the scaler and chosen model are refit on that fold's complete expanding history. The high-volatility threshold is likewise recalculated from the fold's training target, so model selection, transformations, and risk labels never use future rows.

The executed notebook reports aggregate MAE, RMSE, R², and training-defined tail MAE for both regression and the 21-day rolling-volatility baseline. It also saves fold-level metrics, model-selection counts, cumulative out-of-sample MAE, residuals through time, and tail forecast diagnostics under `reports/`. This evaluation assumes older history remains relevant and its fixed initial window and block size are modeling choices, not universal constants. Regime changes, correlated errors, changing variance, vendor revisions, and the absence of portfolio costs remain material limitations.

The executed walk-forward evaluation produced 1,047 out-of-sample forecasts from 2022-06-21 through 2026-08-21 across six folds. Regression beat the rolling-volatility baseline on MAE in all six folds: aggregate MAE was 0.008505 versus 0.009971, a 14.70% improvement, while aggregate R² was 0.070 versus -0.189. Ridge with `alpha=100` was selected in four folds and OLS in two, showing that the preferred regularization is not completely stable through time. More importantly, regression tail MAE was 0.021342 versus the baseline's 0.019707, an 8.30% deterioration. The repeated result strengthens confidence in average-error improvement but confirms systematic underprediction of the large moves that matter most to the risk analyst; the model therefore remains unsuitable for automated risk action.

## Stage 11 Evaluation and Risk Communication

`src/evaluation.py` adds reproducible Gaussian, IID-bootstrap, and circular moving-block-bootstrap intervals for the paired daily MAE difference. Pairing compares regression and baseline on identical forecast days; a negative difference favors regression. The executed notebook uses 5,000 seeded resamples and compares all forecasts, 921 ordinary-volatility observations, and 126 high-volatility observations. It also reports calendar-year subgroups and places scenario MAEs on a consistent axis in `reports/aapl_evaluation_uncertainty_scenarios.png`.

The preferred 20-session moving-block interval acknowledges short-run error dependence. Across all forecasts, the mean paired MAE difference was -0.001466 with a 95% interval of [-0.002183, -0.000924]. On ordinary days, regression improved MAE by 21.88%. On training-defined tail days, however, the difference was +0.001635 with a 95% block-bootstrap interval of [0.000378, 0.002900], and regression MAE was 8.30% worse. Gaussian and IID-bootstrap intervals reach the same directional conclusions, although their narrower all-day intervals illustrate sensitivity to independence assumptions. Regression improved average MAE in every calendar-year subgroup from 2022 through 2026, but the tail result remains the decision constraint.
